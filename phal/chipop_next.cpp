...
void threadStopProc(targeting::TargetHandle proc)
{
    ...
    // MPIPL sequence: enter → continue → stop clocks
    auto rc = hostfw::ipl::dump::enterMpipl(proc, ffdc);
    if (rc != 0)
    {
        std::string msg = "enterMpipl failed";
        if (!ffdc.empty())
        {
            msg += ": FFDC count=" + std::to_string(ffdc.size());
        }
    ...
    ffdc.clear();
    rc = hostfw::ipl::dump::continueMpipl(proc, ffdc);
    if (rc != 0)
    {
        std::string msg = "continueMpipl failed";
        if (!ffdc.empty())
        {
            msg += ": FFDC count=" + std::to_string(ffdc.size());
        }
    ...
    ffdc.clear();
    rc = hostfw::ipl::dump::stopClocks(proc, 0xFF, 0xFF, ffdc);
    if (rc != 0)
    {
        std::string msg = "stopClocks failed";
        if (!ffdc.empty())
        {
            msg += ": FFDC count=" + std::to_string(ffdc.size());
        }
...
void checkSbeState(targeting::TargetHandle target, int)
{
    std::vector<sbei::ResponseInfo::FFDC> ffdc;
    auto rc = hostfw::ipl::dump::getSbeCapabilities(target, ffdc);
    if (rc != 0)
    {
        std::string msg = "SBE not ready";
        if (!ffdc.empty())
        {
            msg += ": FFDC count=" + std::to_string(ffdc.size());
        }
...
void sbeExtractRC(targeting::TargetHandle proc, const std::filesystem::path&, int)
{
    std::vector<sbei::ResponseInfo::FFDC> ffdc;
    auto rc = hostfw::ipl::dump::getTiInfo(proc, ffdc);
    if (rc != 0)
    {
        std::string msg = "getTiInfo failed";
        if (!ffdc.empty())
        {
            msg += ": FFDC count=" + std::to_string(ffdc.size());
        }
...
void finalizeSbeCollection(targeting::TargetHandle target,
                            const std::filesystem::path&, bool success, int)
{
    if (!success)
    {
        return; // Skip quiesce on failure
    }

    std::vector<sbei::ResponseInfo::FFDC> ffdc;
    auto rc = hostfw::ipl::dump::quiesceSbe(target, ffdc);
    if (rc != 0)
    {
        std::string msg = "quiesceSbe failed";
        if (!ffdc.empty())
        {
            msg += ": FFDC count=" + std::to_string(ffdc.size());
        }
