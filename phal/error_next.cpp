...
using FFDCFiles = std::vector<
    std::tuple<DBusFormat, uint8_t, uint8_t, sdbusplus::message::unix_fd>>;
    ...
    try
    {
        ...
        const auto level =
            sdbusplus::xyz::openbmc_project::Logging::server::convertForMessage(
                convertSeverity(entry.getSeverity()));
    ...
    [[maybe_unused]] const std::filesystem::path& dumpPath)
{
    ...
    auto entry =
        errl::ErrlEntry(event, errl::Level::Error, std::move(additionalData));
    if (auto logId = commitEntry(bus, entry); logId != 0)
    {
        logIds.push_back(logId);
    }
