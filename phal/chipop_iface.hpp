 ...
 *
 * Supports malloc-based P10 storage and vector-based phal-next storage.
 */
class DumpData
{
  public:
    ...
    /**
     * @brief Create from a phal-next vector
...
struct ChipOpError : public std::exception
{
    ...
    std::string msg;
    /** Backend-native error object with FFDC ownership. */
    std::shared_ptr<void> native;
    /** Data returned with a non-fatal legacy FFDC indication. */
    std::shared_ptr<DumpData> data;

    ChipOpError(Type t, std::string m) : type(t), msg(std::move(m)) {}
    ChipOpError(Type t, std::string m, std::shared_ptr<void> nativeError,
                std::shared_ptr<DumpData> partialData = {}) :
        type(t), msg(std::move(m)), native(std::move(nativeError)),
        data(std::move(partialData))
    {}
 ...
 *
 * @param chip P10 Proc/Odyssey or P11/PST Hub/OCMB target
 ...
 * Next: hostfw::ipl::sppeControlInstructions()
 *
 ...
 *
 * Calls openpower::phal::dump::initializePdbgLibEkb().
 *
 ...
 *
 * Uses libphal's validated target lookup.
 *
 ...
 *
 * Uses libphal's validated "pib" or "fsi" probe flow.
 *
 ...
 *
 * Calls openpower::phal::sbe::checkSbeState().
 *
 * @param pibFsiTarget  Probed P10 pib/fsi target
 * @param sbeTypeId     SBE_TYPE_PROC or SBE_TYPE_OCMB
 ...
 *
 * Executes the P10 libphal extract-RC flow.
 *
 ...
 *
 * Calls openpower::phal::sbe::collectLocalRegDump().
 *
 * @param proc          Proc/OCMB target
 ...
 *
 * Calls openpower::phal::sbe::collectPIBMSRegDump().
 *
 * @param proc          Proc/OCMB target
 * @param dumpPath      Path to write dump files
 * @param baseFilename  Base filename prefix
 ...
 *
 * Calls openpower::phal::sbe::collectPIBMEMDump().
 *
 * @param proc          Proc/OCMB target
 * @param dumpPath      Path to write dump files
 * @param baseFilename  Base filename prefix
 ...
 *
 * Calls openpower::phal::sbe::collectPPEState().
 *
 * @param proc          Proc/OCMB target
 * @param dumpPath      Path to write dump files
 * @param baseFilename  Base filename prefix
 ...
 *
 * Calls openpower::phal::sbe::finalizeCollection().
 *
 * @param pibFsiTarget  Probed P10 pib/fsi target
