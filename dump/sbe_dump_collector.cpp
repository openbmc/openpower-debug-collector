    ...
    uint64_t failingUnit, uint8_t cstate, const TargetMap& targetMap)
{
    ...
    for (const auto& [primaryTarget, associatedTargets] : targetMap)
    {
        auto future = std::async(
            std::launch::async, [this, primaryTarget, associatedTargets, path,
                                 id, type, cstate, failingUnit]() {
                try
                {
                    this->collectDumpFromSBE(primaryTarget, path, id, type,
                                             cstate, failingUnit);
                }
                catch (const std::exception& e)
                {
                    lg2::error("Failed to collect dump from primary target "
                               "({POSITION}): {ERROR}",
                               "POSITION", phal_tgt::chipPos(primaryTarget),
                               "ERROR", e.what());
                }
...
bool SbeDumpCollector::logErrorAndCreatePEL(
    ...
    phal_tgt::TargetHandle chipTarget, SBETypes sbeType, uint32_t cmdClass,
    uint32_t cmdType, const std::filesystem::path& path)
{
    ...
    catch (const std::exception& e)
    {
        lg2::error("SBE Dump request failed, chip type({CHIPTYPE}) "
                   ...
                   "CHIPTYPE", chipName, "CHIPPOS", chipPos, "ERROR", e.what());
    }
