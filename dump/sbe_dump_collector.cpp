                                      ...
                                      const int sbeTypeId)
{
    ...
    catch (const std::exception& e)
    {
        ...
        // state
        if (proc_ody && (sbeTypeId == PROC_SBE_DUMP || sbeTypeId == ODYSSEY_SBE_DUMP))
            {
                const char* keystr = (sbeTypeId == PROC_SBE_DUMP) ? "pib" : "fsi";
                pibFsiMap[keystr] = probeTarget(proc_ody, keystr, sbeTypeId);
