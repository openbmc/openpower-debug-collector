...
void SbeDumpCollector::collectDump(uint8_t type, uint32_t id, uint32_t failingUnit,
                     const std::filesystem::path& path)
{
    if ((type == SBE_DUMP_TYPE_SBE) || (type == SBE_DUMP_TYPE_MSBE))
        {
            this->collectSBEDump(id, failingUnit, path, type);
        }
        else
        {
            this->collectHWHBDump(type, id, failingUnit, path);
        }
}
...
{
    lg2::info("Collecting SBE dump: path={PATH}, id={ID}, chip position={FAILINGUNIT}",
                      "PATH", dumpPath.string().c_str(), "ID", id, "FAILINGUNIT", failingUnit);    
	...
	try {
        ...
        lg2::error("Failed to collect the SBE dump: {ERROR}", "ERROR", e.what());
