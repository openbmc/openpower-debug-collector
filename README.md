...
```
For local build pass the --prefix=/usr option to the configure script
...
```
For CI build ${datadir} is expanded to /usr/local/share to which the dependent
files and parser are exported, so don't add the prefix option.
```
