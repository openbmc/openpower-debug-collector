...
std::map<uint8_t, DumpTypeInfo> dumpInfo = {{SBE_DUMP_TYPE_HOSTBOOT, {"hbdump_", "/xyz/openbmc_project/dump/hostboot"}},};
...
void Manager::collectDumpFromSBE(struct pdbg_target* tgt, std::filesystem::path &dumpPath, uint64_t timestamp, uint8_t type, uint8_t clockState)
{
    ...
    std::string filename = dumpInfo[type].dumpPrefix + "_" + std::to_string(timestamp) + "_" + std::to_string(chipPos);
    dumpPath /= filename;
    ...
    if ((error = sbe_dump(tgt, type, clockState, &data, &len )) < 0)
    {
        ...
        {
            log<level::INFO>("Failed to collect Hostboot dump from slave SBE, skipping", entry("CHIPPOSITION=%d", chipPos));
            return;
        }
        log<level::ERR>("Failed to collect dump", entry("CHIPPOSITION=%d", chipPos));
    ...
    if (len == 0)
    {
        if ((!isMasterProc(proc)) && (type == SBE_DUMP_TYPE_HOSTBOOT))
        {
            log<level::INFO>("No hostboot dump recieved from slave SBE, skipping", entry("CHIPPOSITION=%d", chipPos));
            return;
        }
        log<level::ERR>("No data returned while collecting the dump", entry("CHIPPOSITION=%d", chipPos));
        throw std::system_error(error, std::generic_category(),
...
void Manager::collectDump(uint8_t type, uint32_t id, std::string errorLogId)
{
    ...
    vector<uint8_t> clockStates = {SBE_CLOCK_ON, SBE_CLOCK_OFF};
    ...
    Manager::createDump(std::map<std::string, std::string> params)
{
    ...
    std::string dumpType = params[sdbusplus::com::ibm::Dump::server::Create::
    ...
    try
    {
        ...
        auto dumpManager  =
            getService(bus1, DUMP_CREATE_IFACE, dumpInfo[type].dumpPath);

        auto method =
            bus1.new_method_call(dumpManager.c_str(), dumpInfo[type].dumpPath, DUMP_CREATE_IFACE, "CreateDump");
        method.append(createDumpParams);
        auto response = bus1.call(method);
        response.read(objPathResponse);
        newDumpPath = std::get<sdbusplus::message::object_path>(objPathResponse);
    }
