...
bool Manager::isSbeBooted(struct pdbg_target* proc)
{
    ...
    void Manager::collectDumpFromSBE(
        struct pdbg_target * proc, std::filesystem::path & dumpPath,
        uint64_t timestamp, uint8_t type, uint8_t clockState)
    {
        using namespace sdbusplus::xyz::openbmc_project::Common::Error;
        using namespace phosphor::logging;
        namespace fileError =
            sdbusplus::xyz::openbmc_project::Common::File::Error;

        int error = 0;
        uint8_t* data = NULL;
        uint32_t len = 0;
        struct pdbg_target* pib = pdbg_target_require_parent("pib", proc);
