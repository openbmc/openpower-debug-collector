...
void handleSbeBootError(struct pdbg_target* procTarget, const uint32_t timeout)
{
    ...
    {
        log<level::ERR>("SBE Boot timeout or No FFDC data");
    ...
    if (dumpIsRequired)
    {
        try
        {
            ...
            if (!dumpAllowed)
            {
                ...
                log<level::INFO>("SBE dump collection not allowed");
                return;
            }

            // Check SBE vital attention active
            bool sbeVitalAttnActive = pdbg::isSbeVitalAttnActive(procTarget);
            if (sbeVitalAttnActive)
            {
                // HW diag handles SBE dump collection, skip dump collection.
                log<level::INFO>(
                    "SBE vital attention active, skipping SBE dump collection");
                return;
            }
        }
        catch (const std::exception& e)
