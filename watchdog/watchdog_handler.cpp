...
std::function<void()> alarmTimeoutHandler;
...
void monitorDump(const std::string& path, const uint32_t timeout)
{
    ...
    auto timeOut{false};
    alarmTimeoutHandler = [&timeOut]() {
        timeOut = true;
        alarm(0); // Cancel alarm
    };

    signal(SIGALRM, alarmSignalHandler); // Register signal handler
    alarm(timeout);
    ...
    auto timerCallback = [&](Timer& /*source*/, Timer::TimePoint /*time*/) {
        timeOut = true;
        log<level::ERR>("hbdump Timer callback!!!");
	return;
    };

    Timer time(event, (Clock(event).now() + std::chrono::seconds{10}),
